...
- **가맹점 업종 구분 시스템**: 자체 DB 캐시(TTL 30일) → 카카오 지도 API 원시조회 → **LLM이 카카오 원시 카테고리를 우리 서비스 업종 어휘로 재분류**(카카오 group code는 장소·마케팅 분류라 그대로 안 씀, 웹검색 폴백 없음 — 카카오·LLM 둘 다 실패하면 업종 미확정으로 남김) → 비용분류 **보조 힌트**로만 사용(세무 판단 아님). 표준 업종코드(MCC)는 카드사 제휴 **post-MVP** 확장. (기술 §7-1 / 요구사항 §6.5 / FR-DA-03a~c. **구현은 `classify_merchant` MCP Tool까지 — Draft/Risk Agent 연동은 미착수**, 아래 상태 보드 참조)
...
## 3. 상태 보드 (Status Board) — _최종 갱신: 2026-08-18_
...
| 가맹점 업종 구분 시스템 | 🚧 Tool 실동작 / Agent 연동 미착수 | `classify_merchant` MCP Tool 실구현(2026-08-18) — 정규화 → Django 캐시(`merchant_categories`, **TTL 30일**, `MerchantCategoryLookupView`/`UpsertView` 신설) → 카카오 로컬 API 원시조회 → **LLM 재분류**. 카카오 원시 `category_group_code`/`category_name`(장소·마케팅 분류, 예: 일반식당과 유흥주점이 둘 다 FD6)을 그대로 쓰지 않고, `app/merchant/classify.py::_llm_classify`가 가맹점명+카카오 원시 카테고리를 보고 우리 서비스 고정 어휘 10종(일반음식점/카페/주점·유흥/숙박/레저·골프/마트·편의점/문구·사무용품/주유·교통/전자·가전/기타)으로 재분류 — "주점/유흥"을 "일반음식점"과 분리한 게 핵심(Risk 리스크 판단에 실제로 쓰이는 구분, 카카오 group code만으로는 못 가름). 카카오 미스·LLM 실패 둘 다 원시값으로 새지 않고 미확정(공란) 반환(예외 아님). 캐시 쓰기는 `IngestCallbackView`와 동일하게 ai 서비스 계정(capability `rule_view`) 인증 필요. 실측: 캐시 히트(시드 데이터 "스타벅스"→CACHE) 확인. **Kakao Developers 콘솔에서 "카카오맵" 제품 활성화 완료(2026-08-18)** — 캐시 미스 건("오늘의포차 역삼점")으로 실키 재검증: 카카오 원시조회 200 정상 응답(`category_group_code=FD6`) → LLM이 "주점/유흥"(confidence 0.9)으로 정확히 재분류, `source=KAKAO`까지 캐스케이드 전 구간 실통과 확인(직전 세션의 403 미활성 이슈 해소). Django 테스트 5건·FastAPI 테스트 10건 통과. **Draft/Risk Agent 연동은 이번 범위에서 제외**(사용자 지시, 2026-08-18) — Tool만 우선 실동작, 언제 어느 Agent가 부를지는 후속 과제 |
