...
├── docs/                ← 【팀 관리】 기준문서 — SoT. 에이전트는 읽기 기본
│   ├── 요구사항_명세서.md   ┐ 권위(authored) 스펙 3종
│   ├── 기술명세서.md        │  (권위 범위·버전·상태는 _index.md 표에서 관리)
│   ├── 기획_확장안_v2.md    ┘
│   ├── 법인카드_사용규정_기반_RULE_명세서.md  ← 참고용 예시(권위 아님)
│   └── 비지도모델_비교실험_결과보고서.md
├── _context/            ← 【AI 관리】 에이전트 생성 파생 컨텍스트 (캐논·설계·구현 추적)
├── 화면설계서/           ← 압축해제 .docx (본문 word/document.xml, 추출 레시피는 CLAUDE.local.md)
└── figma_mockup/         ← 화면 목업 SVG (참고용, 픽셀 매칭 불필요)
```

**소유권**: `docs/`는 **팀이 합의해 관리하는 기준문서**, `_context/`는 **AI가 관리하는 AI용 컨텍스트**. 둘이 어긋나면 `docs/`가 이긴다. 발표·스크럼 자료는 llm_wiki 밖 `scrum/`.
