...
| 적재 → 룰 자동 생성 트리거 | 🔩 틀만 | `apps/ai/app/rag/rule_trigger.py` — **연결 지점만 잡고 호출 시 "개발 중" 안내를 반환**한다(결과는 `PolicyDoc.rule_trigger`에 저장돼 화면에 노출). 순서 의존이 핵심: 적재 완료 **후**에 불려야 하고(그 전이면 검색 0건→`NO_SOURCE`), 같은 백그라운드 태스크가 체인을 소유해 그 순서가 보장된다. 켜기 전 결정 2건 — ① 범위(업로드 시 고른 scope 1개 vs 문서에서 탐지된 전 scope: 후자는 LLM 호출 곱절 + 미요청 DRAFT 누적) ② 재색인 때도 돌릴지(매번 새 계열이 생기면 룰 콘솔이 초안으로 뒤덮인다 — 기존 계열에 버전 얹는 경로 선행 필요) |
