...
`llm_wiki/`는 이 프로젝트 컨텍스트의 **단일 진실 원천(SoT)**. 에이전트/Codex는 **`llm_wiki/_index.md`(색인)를 세션 시작 시 먼저 읽고**, 컨텍스트를 바꾸면 해당 문서와 색인 행을 함께 갱신한다.
...
├── 기획_확장안_v2.md     ┘
├── 화면설계서/           ← 압축해제 .docx (본문 word/document.xml, 추출 레시피는 Codex.local.md)
...
# 프론트 (apps/web) — 이 머신에선 --prefix 절대경로 권장(§Codex.local.md)
...
- 개인 로컬 환경 노트·명령 tip은 `Codex.local.md`(git 미추적) 참고.
