...
def _run_rows(snapshot: dict, cases: list[dict[str, Any]], source: str) -> list[dict[str, Any]]:
    ...
    for index, case in enumerate(cases):
        ...
        comment, verdict = _change_comment(baseline, result.decision, case) if changed else ("", "")
        rows.append({
...
def _change_comment(baseline: str, decision: str, case: dict[str, Any]) -> tuple[str, str]:
    """분류가 달라진 건에만 붙는 짧은 AI 코멘트 — (문구, 판단)."""
    before, after = DECISION_KO.get(baseline, baseline), DECISION_KO.get(decision, decision)
    if baseline == "REVIEW" and decision == "PASS":
        return (f"사람이 검토하던 건을 그래프가 자동 {after} 처리했습니다. "
                "의도한 자동화 효과로 보이나, 통과 경로의 조건이 실제 승인 기준과 같은지 표본 확인을 권장합니다.", "intended")
    if baseline == "REVIEW":
        return (f"검토 대기 상태였던 건에 그래프가 «{after}» 판정을 내렸습니다. "
                "판정 근거 노드가 규정과 맞는지 확인하세요.", "intended")
    if baseline == "PASS" and decision in RISK_DECISIONS:
        return (f"기존에 {before}된 건이 «{after}»로 바뀌었습니다. 규정 근거가 명확하지 않다면 "
                "과탐(오탐)으로 검토량만 늘어날 수 있습니다.", "risk")
    if baseline in {"RETURN", "REJECT"} and decision == "PASS":
        return (f"기존에 {before}된 건이 «{after}»로 바뀌었습니다. 놓친 위반이 없는지 반드시 확인하세요.", "risk")
    if case.get("aiSuggested"):
        return (f"«{before}» → «{after}»로 바뀌었고, 비용 분류 자체가 AI 저신뢰 추천 건입니다. "
                "분류부터 다시 확인하세요.", "risk")
    return (f"«{before}» → «{after}»로 판정이 바뀌었습니다. 변경 사유가 의도한 규정 변화인지 확인하세요.", "intended")
...
def simulate(graph: RuleGraph, test_cases: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "structure": shape,
        "snapshot": snapshot,
        "agentReport": _agent_report(graph, shape, stats, grades, test_rows, history_rows, structure_error, period_label),
...
def _grades(shape: dict, stats: dict, structure_error: str) -> dict[str, Any]:
    ...
    if structure_error:
        ...
        structure = ("poor", f"도달 불가 노드 {len(shape['unreachable'])}개")
    elif stats["nodeCoverage"] < 0.8:
        structure = ("warn", f"노드 커버리지 {stats['nodeCoverage'] * 100:.0f}%")
    else:
        structure = ("good", f"노드 {shape['nodeCount']}개 · 깊이 {shape['maxDepth']}단계 · 미도달 없음")
...
def _agent_report(graph, shape, stats, grades, test_rows, history_rows, structure_error, period_label) -> str:
