...
def _dsl_grammar(_params: dict) -> dict[str, Any]:
    ...
    return {
        "data": {
            ...
            "value_operator": "var",
            # 모름을 참으로 만들 수 있는 유일한 연산자. **생성 툴 스키마에는 없다**
            # (`op` enum이 닫혀 있어 모델은 낼 수 없다) — 사람이 룰 콘솔에서 쓴다.
            "null_test": dsl.NULL_TEST,
        ...
        "notes": [
            ...
            "**모름(null)은 어느 방향으로도 참을 만들지 않는다.** 비교에 null이 하나라도 끼면 "
            "결과는 항상 거짓이다 — `==`도, `!=`도, 대소 비교도, `in`도. `not(x)`도 x가 null이면 "
            "거짓이다(모름의 부정은 참이 아니다).",
            "그래서 「~가 아니면 걸린다」를 `!=`로 써도 **모르는 건은 안 걸린다** — 대신 엔진이 "
            "그 건을 「판단에 필요한 정보 없음」으로 검토에 넘긴다. 값을 모를 때 무슨 일이 "
            "일어날지 따로 설계하지 않아도 된다.",
            f"`{dsl.NULL_TEST}`만이 모름을 참으로 만든다(단항 — 좌변 하나만 받는다). "
            "**「값이 없다/아니다」와 「값을 모른다」는 다른 사실이다** — 「받지 않았다」는 "
            "`== false`, 「적혀 있지 않다」는 `is_null`이다. 모르는 경우를 따로 처리할 필요는 "
            "없으니(위 규칙대로 자동으로 검토로 간다) **모름 자체를 사유로 삼고 싶을 때만** 쓴다.",
        ],
    }
