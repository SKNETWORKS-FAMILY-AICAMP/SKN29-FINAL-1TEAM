"""데모 데이터 시드 — 프론트 연동/통합 테스트용.

    docker compose exec core python manage.py seed
멱등하지 않으므로(중복 생성) 초기화하려면 `seed --fresh` 사용.
"""
...
from domain.policies.models import (
    OnResult, RuleGraph, RuleGraphStatus, RuleNode, RuleRouting,
)
        ...
        for i, (merchant, amount, c, cat, ai, status, ev) in enumerate(rows):
            ...
            if status == SettlementStatus.IN_REVIEW:
                RiskReview.objects.create(
                    ...
                    reasons=[{"feature": "심야 사용", "weight": 0.34}, {"feature": "한도 초과", "weight": 0.29}],
                    rag_refs=[{"title": "접대비 50만원 초과 사전결재", "source": "TIGER-REG-2026-003 §12조"}],
