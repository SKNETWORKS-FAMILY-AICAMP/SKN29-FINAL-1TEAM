...
export function SettlementDetailModal({
  ...
  //  **신규 등록(item===null)에선 애초에 안 쓰인다** — 그런데 그냥 두면 아래 삼항이
  //  `isOwner`(신규 등록은 항상 true)를 타고 내려가 `item!.status`까지 도달해, item이
  //  실제로 null인 채로 `.status`를 읽어 렌더가 통째로 죽는다(실측: 신규 지출 등록 시
  //  화면 전체가 에러 경계로 대체됨). 값 자체를 안 쓰더라도 계산은 매 렌더 일어나므로
  //  isCreate를 가장 먼저 걸러야 한다.
  const lockNote = isCreate
    ? ''
    : !isOwner
      ? (canTeamDecide
