...
import { fetchDecisionReason } from '../../api/settlementService'

//  서버가 선택지를 못 줬을 때만 쓰는 최소 폴백. **정본은 서버**
//  (`domain/settlements/decision_reasons.py`)다 — 화면과 LLM이 같은 목록을 봐야 한다.
const FALLBACK_REASONS: Record<'RETURN' | 'REJECT', string[]> = {
...
export function DecisionReasonModal({
  ...
  const footer = (
    <>
      ...
      {isReject ? (
        <button className="btn reject" onClick={() => setConfirming(true)} disabled={busy}>반려 전송 (REJECT)</button>
      ) : (
        <button className="btn return" onClick={() => onConfirm(reason, detail)} disabled={busy}>
          {busy ? '처리 중…' : '보완요청 전송 (RETURNED)'}
        </button>
      )}
    </>
  )
  ...
  return (
    <Modal title={isReject ? '반려 사유 입력' : '보완요청 사유 입력'} onClose={onClose} footer={footer}>
      ...
      <div className="field" style={{ marginBottom: 0 }}>
        <label>상세 사유 {isReject ? '(선택)' : '— 지출자가 무엇을 하면 되는지 적어주세요'}</label>
        <textarea
          rows={4}
          placeholder={
            isReject
