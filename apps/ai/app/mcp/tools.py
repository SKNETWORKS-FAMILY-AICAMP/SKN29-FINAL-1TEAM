...
def search_policy(query: str, filters: dict | None = None) -> dict:
    """규정 청크 RAG 검색 (Chroma 직접, `policy_docs`). Rule/Risk.

    운영 인덱스(`app/rag/embedding/store.py`, `policy_docs` 103청크 실적재 확인됨)를 그대로
    쓴다 — `agents/rule_agent_v0/vector_store.py`는 격리된 로컬 실험 스토어(1건)라 여기서
    쓰지 않는다(그걸 그대로 승격하면 2차 검증이 빈 결과만 도는 죽은 경로가 된다).
    """
    from app.rag.embedding import store as rag_store

    hits = rag_store.search(query, collection_name="policy_docs", top_k=6)
    if filters:
        hits = [h for h in hits if all(h.get("metadata", {}).get(k) == v for k, v in filters.items())]
    chunks = [
        {
            "chunk_id": h["chunk_id"],
            "citation": h.get("citation") or h["chunk_id"],
            "text": h.get("parent_document") or h["document"],
            "score": h["score"],
        }
        for h in hits
    ]
