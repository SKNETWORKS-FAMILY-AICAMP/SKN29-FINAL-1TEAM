...
# 보완요청·반려 사유 초안 — 결정 모달이 열릴 때 Django가 부른다.
app.include_router(decision.router, prefix="/agent", tags=["agent"])
...
try:
    ...
    # requirements.txt가 fastmcp==2.4.0으로 고정되면서 `http_app()`(Streamable HTTP)이
    # 다시 쓰인다 — 2.1.2 시절엔 이 메서드가 없어 `sse_app()`(SSE 트랜스포트)로 우회했었다.
    # 참고: Rule Agent 등 같은 프로세스 안에서 도구를 부르는 내부 호출자는 이 HTTP 마운트를
    # 거치지 않고 `fastmcp.Client(mcp)`(in-process 트랜스포트)로 직접 붙는다 — 이 마운트는
    # 외부 MCP 클라이언트(예: Claude Desktop)를 위한 것.
