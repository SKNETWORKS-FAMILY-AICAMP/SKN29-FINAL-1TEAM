...
> 이 문서는 **실제로 무엇을 만들었고, 어떻게 검증했는지**를 다룬다. 브랜치: `feature/rule-agent-v1`.
> §0에 "무엇을 결정했는지"(구 `agent-v1-upgrade-plan.md`, 2026-08-25 이 문서로 병합) 요약이 있고,
> §1 이후가 그 결정의 근거·코드 위치·실동작 검증 결과다. §0 6개 항목 전부 이 문서 안에서 찾을 수
> 있다 — ①검증재사용(§3) ②재생성루프(§1~4) ③자동트리거(§7) ④대화형에이전트(§8) ⑤MCP툴콜링전환(§6) ⑥시뮬레이션LLM서술(결정: 안 함, 코드변경없음).

## 0. 결정 배경 — v1 고도화 계획 요약 (2026-08-19 확정)

원칙: **미구현 항목을 켜는 작업이 이미 동작 중인 v0 경로를 절대 깨지 않는다.**
Rule Agent §0.1의 6개 항목 **전부 구현+실동작 검증 완료** — `feature/rule-agent-v1` →
`feature/doc-manage` 경유 → `main` 병합됨.

### 0.0 MCP — 배관과 루프는 다른 문제였다

MCP 서버는 등록돼 있었지만 실제로는 두 겹으로 깨져 있었다(툴 등록 API 오용 + 존재하지 않는 마운트
메서드 호출). 아무도 이 마운트를 거치지 않고 우회하고 있어서 오래 방치됐던 것 — 상세 원인·수정·
검증은 **§6.1**.

Rule Agent는 v1에서 이 마운트를 고치고, `fastmcp.Client`로 실제 MCP 프로토콜을 태워
`search_policy`를 호출하며, LLM 호출 자체를 멀티턴 tool-calling 루프로 재작성했다(§0.1-1).
Risk Review Agent는 당시 아직 손대지 않아 직접 함수 호출 상태였다(이후 상태는
`risk-review-agent-v1-implementation.md` §0 참조).

### 0.1 Rule Agent — 결정 상태 (6개 항목 전부 확정·구현 완료)

| # | 항목 | 결정 | 구현·검증 |
|---|---|---|---|
| 1 | MCP 툴콜링 재작성 | 전체 재작성(마운트 수정+in-process 배선+진짜 멀티턴 루프) | §6 |
| 2 | 적재→생성 자동 트리거 | scope는 업로드 시 선택 1개만, 재색인 시엔 생성 안 함 | §7 |
| 3 | 그래프 검증 | 새 MCP 툴 신설 대신 기존 `POST /rules/{id}/simulate` 재사용. **워크플로우 순서 결정(2026-08-18)**: "①테스트검증" 단계는 이제 **검증셋 자동생성(§0.4)이 먼저, 그다음 시뮬레이션 실행**이 권장 순서 — 사람이 빈 검증셋에 수동으로 하나씩 채우던 이전 순서 대신, 자동생성이 커버리지 있는 검증셋을 먼저 채워두고 사람은 그 위에 추가/수정만 하는 흐름으로 정정. **프론트 연동 완료**(버튼 클릭→생성→결과 화면 갱신) | §3, §6.3, §12, §13.1 |
| 4 | 검증→재생성 루프 | A안(저장→검증→실패시 discard), 최대 3회, RAG 청크는 outer 재시도 전체에서 재사용, 신규 종료 상태값(`NO_VALID_NODES_EXHAUSTED`/`STRUCTURE_INVALID_EXHAUSTED`) | §1~5 |
| 5 | 대화형 자연어 수정 에이전트 | 신규(`chat.py`), 기존 그래프 CRUD 3종 재사용, `RuleAuthoringMessage` 최초 실사용. **후속 수정(2026-08-18)**: 선택 노드·대화 이력 미전달 버그 수정 | §8, §13.2 |
| 6 | 시뮬레이션 보고서 LLM 서술 | 최초 팀 결정은 "추가 안 함"이었으나, 실사용 중 화면에 "플레이스홀더 보고서" 태그가 그대로 노출되는 게 확인돼 **재검토 후 구현 완료(2026-08-18)** — 통계·판정(실데이터)은 그대로 두고, 서술문만 신규 `narrate-report` 엔드포인트(단일 LLM 호출)로 교체. LLM 실패 시 기존 룰 기반 템플릿으로 자동 폴백 | §13.3 |

구현 후 전수 검토에서 추가로 잡은 버그 3건 + docling 버전 드리프트로 실업로드 최종 고리가 막혀 있는 상태는 §10·§11 참조.

### 0.2 비침습 체크리스트 (신규 항목 착수 전 재확인용 — 방법론, v1에서 전부 통과 확인됨)
...
### 0.3 프론트 연동 작업과의 격리 전략

**배경**: 프론트-에이전트 연동을 다른 팀원이 병행 작업했다(main 병합 완료). 격리 지점은 "코드
위치"가 아니라 **계약(contract)** — 프론트가 실제로 붙는 건 Django API 엔드포인트지 `apps/ai`
내부 함수가 아니다.

**레포 전례**: `Review List v0`를 만들 때 운영 모델은 그대로 import하되 별도 서브패키지+별도
라우트로 격리했다(이후 `settlements/risk_review.py`로 정식 편입). v1도 동일 패턴 — 신규
엔드포인트(`/converse` 등)로 노출하고 기존 엔드포인트 응답 shape는 동결.

**주의**: 적재→생성 자동 트리거(§0.1-2)는 코드상 켜져 있지만, 당시 docling 버전 드리프트
버그로 실제 업로드 파싱이 막혀 있어 실전에서 발동하지 않았다(상세: §10). 그 버그는 이후
버전 고정으로 해소됐다(CLAUDE.md §3.4 참조).

### 0.4 테스트케이스 자동생성 — ✅ 구현+검증 완료 (2026-08-18)

**배경·요구사항**: 룰 콘솔에 테스트케이스(`RuleTestCase`)를 **사람이 수동으로 만드는 화면**은
이미 있고 실제로 쓰이고 있었다. 여기에 **에이전트가 자동으로 만들어주는 기능**을 추가했다.
대화형이 아니다 — §0.1-5(대화형 자연어 수정)와 다른 성격. 사용자가 그 자리에서 다듬는 게
아니라 **에이전트가 완제품을 한 번에 내놓고 사람이 검토·사용**하는 흐름이라, 생성 결과 자체의
품질이 담보돼야 한다는 게 핵심 제약이었다.

**방향 — 두 축**:
1. **커버리지 기반 생성(자유생성 지양)**: 노드 조건(JSON-Logic DSL)을 역산해서 "이 노드를 걸리게
   하는 값"을 결정론적으로 만든다 — LLM에게 "적당히 만들어봐"라고 맡기지 않는다.
2. **자체 검증 루프**: 후보 케이스를 만든 뒤 실제로 그 케이스가 의도한 노드에 걸리는지 자체
   실행해 확인한다. 안 걸리면 값을 고쳐 재시도. **[구현 시 정정]** MCP `run_rule_engine`은 실제
   Settlement+ACTIVE 그래프 대상이라 DRAFT 그래프의 가상 케이스 검증엔 안 맞았다 — 대신
   §0.1-3의 `simulate_graph()`(기존 `/simulate` 재사용)를 그대로 썼다.

**확정된 것**: 기존 커스텀 케이스는 안 건드린다 — **추가(append) 방식**(단, `PUT
/api/rules/{id}/test-cases/`는 통째로 교체라 AI 쪽이 먼저 `GET`으로 읽어 합친 뒤 다시 씀).
**노드당 2건**: ①확실히 조건을 충족하는 값(MATCH 확인용) + ②경계값(흔한 `>`/`>=` 버그를
잡는 용도) — 검토 부담 대비 이득으로 판단.

**구현 시점 결정**: 자체검증 재시도 상한 **최대 2회**(결정론적 역산이라 §0.1-4의 3회보다 적게
잡음) · append 시 키 충돌은 **`AUTO-N` 접두사**로 발급 · 생성 직후 자동 `/simulate` 실행해
응답에 `simulationReport` 항상 포함.

코드 위치·검증 결과는 §12.

---

## 1. 변경 파일 (검증→재생성 루프, §0.1 항목3·4)
...
- 이걸로 §0.1 항목4의 타임아웃 증가(120s→300s)를 포함해 실제 진입점 전체가 동작함을 확인.
...
**기존 "RAG 청크는 outer 재시도 3회 전체에서 재사용" 결정(§0.1 항목4)과의 정합**: outer 루프가 매 attempt마다 새로 만들지 않고, `generate()`가 최초 1회만 MCP로 검색한 청크(`initial_chunks`)를 매 attempt의 대화 맥락에 고정으로 심는다. 모델이 그걸로 부족하다고 판단할 때만 **대화 안에서** 추가로 `search_policy`를 호출한다 — outer 재시도 간의 재사용 보장은 유지하면서, 한 attempt 내부에서는 진짜 에이전틱하게 검색할 수 있게 절충했다.
...
## 8. 대화형 자연어 수정 에이전트 (§0.1 항목5)

### 8.1 설계

§0.1 항목1에서 만든 MCP 툴콜링 패턴을 그대로 재사용 — 신규 모듈 [`apps/ai/app/agents/rule_agent_v0/chat.py`](../../apps/ai/app/agents/rule_agent_v0/chat.py):
...
- 대화 로그를 `django_client.post_messages()`로 `RuleAuthoringMessage`에 남긴다 — **이 테이블은 예전부터 있었지만 실제로 쓰는 코드가 없었다**(§0.1 항목5 설계 시점 실측). 이번이 첫 실제 쓰기 경로.
...
**남은 것**: 프론트 화면에서 실제 버튼/채팅 UI로 눈으로 확인하는 것(API 레벨까지만 검증, §0.3 계약상 프론트 작업은 별도), Risk Review Agent 쪽 MCP 전환(`risk-review-agent-v1-implementation.md` §0 참조, 이후 완료), 원격 브랜치 push 여부.
...
- Django `_dispatch()` → `POST /embeddings/ingest`(FastAPI) 호출 → 백그라운드 태스크 기동까지 정상(§0.1 항목2에서 추가한 `isReindex` 필드도 정상 전달, 이 경로 자체는 문제 없음)
...
**결정(2026-08-16, 팀 확인)**: 지금은 넘어간다 — 이 버그는 파싱 파이프라인 자체의 문제라 별도로 다룰 사안. 자동 트리거(§0.1 항목2, §7)의 **트리거 판단 로직 자체**(`SKIPPED_REINDEX`/`SKIPPED_NO_SCOPE`/실제 `generate()` 호출)는 §7.3에서 이미 직접 호출로 검증됐고 유효하다 — 이번에 막힌 건 "실제 업로드→파싱까지 끝나야 트리거가 불린다"는 **선행 조건(파싱 성공)** 쪽이지, 트리거 로직 자체가 아니다.
...
## 12. 검증셋(테스트케이스) 자동생성 — 구현 (2026-08-18, 배경·결정은 §0.4)
...
- **축2(자체 검증 루프) — "기대값 vs 실행 결과" 비교 방식**: 이 기능의 핵심 메커니즘이라 명시적으로 적어둔다.
  ...
  2. **실제값은 진짜 룰 엔진을 돌려서 얻는다**: 후보를 실제로 저장하고 `django_client.simulate_graph()`(§0.1-3, 기존 `/simulate` 재사용)를 호출 — 이건 에이전트가 만든 그래프를 Django의 실제 결정론적 엔진(`engine.py::run_rule_engine`)으로 진짜 실행시키는 것이다. **[설계 문서에서 정정]** §4.2가 지목했던 MCP `run_rule_engine`은 실제 Settlement+ACTIVE 그래프 대상이라 DRAFT 그래프의 가상 케이스엔 못 쓴다는 게 구현 중 드러나서 이걸로 바꿨다.
...
자체검증 재시도 2회(최초+1), append 키는 `AUTO-N`, 생성 직후 `/simulate` 자동 실행해 응답에 `simulationReport` 포함 — 전부 §0.4에 최종 결정으로 반영.

### 12.7 v1 스코프 한계 (의도적으로 안 만든 것)

- 조건이 `var op 리터럴`형 비교의 and/or/not 조합이 아니면(예: 산술식) 생성 자체를 건너뛴다 — LLM 폴백으로 억지로 값을 만들지 않는다.
- 자체검증 실패 시 LLM에게 "왜 실패했는지" 피드백을 주고 다시 만들게 하는 건 안 만들었다(§0.1-4·§0.1-5의 피드백 루프와 다른 점) — 결정론적 역산이 실패하는 원인(주로 상위 노드 선점, §12.3)은 "가치가 조금 더 큰 값을 시도"하는 식으로 LLM이 반복해서 고칠 수 있는 성질이 아니라고 판단해서, 대신 재시도 1회(마진 조정) 후 실패로 보고하는 쪽을 택했다.

### 12.8 후속 정리 (2026-08-18)

- **워크플로우 순서 결정**: "①테스트검증" 단계는 이제 **검증셋 자동생성이 먼저, 사람의 수동 보정은 그다음**이 권장 순서로 정정됐다(§0.1 항목3). `/simulate`가 자동으로 생성을 먼저 트리거하게 만드는 건 하지 않기로 함(명시적으로 확인됨) — 화면에 별도 "검증셋 자동생성" 버튼(§13.1)을 두는 쪽으로 확정.
