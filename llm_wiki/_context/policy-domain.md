...
고정 카탈로그의 최대 이점은 **결측을 계산할 수 있다**는 것이다.

```
미해소 = (그래프가 참조하는 policy.* 경로) − (조립기가 실제로 채운 경로)
```

- 미해소가 하나라도 있으면 판정을 `PASS`로 내리지 않고 **`REVIEW`로 강등**한다.
- `RuleHit.flags`에 `UNRESOLVED_POLICY_VAR:<field>`를 남긴다.
- 검토 화면(S-03)의 EvalContext 스냅샷에서 해당 필드를 결측으로 표시한다.

**이 가드는 저장층·조립기보다 먼저 넣을 수 있고, 넣는 순간 현재의 최대 버그가 가시화된다.**
