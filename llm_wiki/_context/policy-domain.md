...
> 읽는가"의 단일 캐논이다. 구현 순서·인수 조건 실측 기록은 §8.
>
...
| `ctx.policy.*` 8개 카탈로그 | 룰엔진 설계(`rule-engine.md` §2) 단계 | 별표 룩업을 선해소한 스칼라 |
...
설계 단계에서 **명시적으로 기각한 방향**이다(DSL 복잡화 + 엔진으로 I/O 누수 — `rule-engine.md` §8 참조).
...
### 3.3 별표 적재 파이프라인 (2026-08-22 구현)

문서 업로드가 별표를 표로 뽑아 **승인 대기 후보**로 만든다.

    적재 완료 ─► triage(`app/rag/triage.py`) ─► PolicyTableProposal(PENDING)
                                              └─► 규정 문서 화면에서 승인 ─► PolicyTable

**후보는 `PolicyTable`이 아니라 별도 모델(`PolicyTableProposal`)이다.** `load_tables()`는
필터 없이 전부 읽어 `ctx.policy.*`로 선해소하므로, 같은 테이블에 미승인 행이 섞이면 status
필터를 잊은 한 곳 때문에 **검증되지 않은 임계값이 조용히 판정에 들어간다**. 모델을 갈라 두면
그 실수가 불가능하다 — `PolicyTable`에 있다는 것이 곧 "사람이 승인했다"는 뜻이 된다.

**승인이 강제하는 것은 축뿐이다**(`table_proposals.validate`): 스키마에 없는 축은 **거부**한다.
경고로 두면 승인 버튼을 누르는 사람이 읽고 넘길 수 있고, 넘기면 그 순간부터 §3.2의 조용한
와일드카드가 된다. 값이 원문과 맞는지는 시스템이 알 수 없으므로 화면이 **표 원문을 나란히**
보여주고 사람이 대조한다 — 승인은 "내가 대조했다"는 서명이다.

**개정은 승인이 INSERT로 처리한다** — 같은 key에 새 `effective_date` 행을 넣고 구행에
`superseded_date`를 찍는다. 과거 지출은 그 시점 한도로 재현된다(회귀로 고정).

**재색인은 사람이 처리한 것을 지우지 않는다** — 승인·반려된 제안은 남기고 `PENDING`만 교체한다
(`source_chunk_id` 기준). 반려를 지우면 같은 표가 매번 승인 대기로 되살아나 같은 판단을
반복하게 된다. 반려에 사유를 강제하는 것도 같은 이유다.
...
> 리네임은 임포트 파급이 크므로 범위 밖(§8.3 비목표).

---

## 8. 구현 이력 — PLAN과 인수 조건 실측 기록

> 이 섹션은 `policy-domain-plan.md`(구현 계획서)를 2026-08-25에 흡수한 것이다.
> 위 §1~7이 **현재 구조**이고, 아래는 착수 전 계획과 완료 시점(2026-08-11) 실측 검증 기록이다.

### 8.1 무엇을 했나 (요약)

1. `engine.run_rule_engine`에 미해소 가드 추가(참조한 `policy.*`가 안 채워지면 `REVIEW` 강등)
2. `EvalContext` 카탈로그 재정의(스키마 v1→v2), 상수 박힌 필드명 정리
3. `PolicyTable` 모델 신설 + 별표 적재, 기존 `Policy`(참조 0) 폐기
4. `policies/context_builder.py` 조립기 구현 — `RESOLVERS`로 별표 해소, `build_rule_context(tx_id)` 연결
5. `draft_agent.THRESHOLDS`·시드 DSL의 하드코딩 임계값 제거 → `PolicyTable` 참조로 일원화

### 8.2 인수 조건 (Definition of Done) — ✅ 전부 실측 통과 (2026-08-11)
...
- `domain/policies` → `domain/rules` 앱 리네임(임포트 파급 큼)
- Chroma 임베딩·upsert 실구현
- 지도학습·자동 재학습(post-MVP)
- 규정 원문 자동 파싱으로 별표 JSON 생성(당시는 수동 적재)
