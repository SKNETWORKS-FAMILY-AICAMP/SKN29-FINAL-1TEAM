...
### 17.5 LLM 노출 — 구조가 이미 막고 있다

`is_null`은 카탈로그에 **실린다**(프롬프트가 말하는 것과 검증기가 강제하는 것이 갈리면 안 된다).
다만 생성 툴 스키마의 `op` enum이 `["==","!=",">",">=","<","<=","in"]`로 **닫혀 있어 모델은
낼 수 없다** — 단항 표현 자체가 없다. 카탈로그 note에 "이 연산자는 생성 도구에 없으니 여기서
쓰지 마라, 모름이 필요하면 `*_resolved` 관측 불린을 쓰거나 그 룰을 건너뛰어라"를 명시했다.

**왜 굳이 막나**: `is_null`이 붙은 경로는 가드가 면제된다. 모델이 「사전승인 안 받음」을
`is_null(approval.pre_approval_obtained)`로 쓰면 — 그 필드는 화면 미입력 시 None이라 흔하다 —
**미입력이 곧 위반 판정이 되고 가드가 그걸 못 막는다.** 정확히 T-21이 저지르던 실수를, 이번엔
안전망 없이 저지르는 꼴이다. 사람이 룰 콘솔에서 쓰는 것으로 족하다.

> 열 수 있게 되는 조건: 카탈로그가 **필드별로 「항상 채워짐 / 모를 수 있음」을 함께 실으면**
> 모델이 판단 근거를 갖는다. 지금 `FieldSpec`에는 그 정보가 없다(조립기 지식이라 스키마가
> 모른다). 그걸 넣는다면 그때 노출을 재검토할 만하다.
