...
### D-5a. `비품` 폐기 (2026-08-24)

과목을 **회식·회의·식대·출장·접대** 다섯으로 단순화하면서 `비품`을 뺐다. 소모품·사무용품
구매는 「나열된 어디에도 안 맞는」 지출이라 `기타`가 그 자리를 대신한다.

**빼는 방향이라 이관이 먼저다** — `RuleGraph.scope` CHECK 제약이 좁아지므로 옛 값을 쓰는
행이 남아 있으면 `AddConstraint`가 죽는다(§6 체크리스트의 그 경우다).

| 순서 | 무엇 |
|---|---|
| ① `settlements/0015` | 정산 `category`·`ai_category`, 팀예산 행을 `기타`로. **팀예산은 같은 (팀,월)에 기타 행이 있으면 한도를 합친다**(불변식: 총한도 = 과목 합) |
| ② `policies/0021` | `비품` scope 그래프를 `기타`로 옮긴 **뒤** 제약을 좁힌다. `settlements/0015`에 의존을 명시했다 |

**`policies/scope.py`의 `"비품"` 매핑은 되살리지 않는다.** 규정 문서에 「비품」 과목이 나와도
그 자리를 대신할 정본 값이 없으므로 매핑하지 않고 원문을 통과시킨다 — 임의로 `기타`에 붙이면
"비품 규정"으로 만든 그래프가 조용히 기타 과목을 판정한다.

### D-5. `기타`를 추가한다 — 단, **`기타` ≠ 미기재**

| 값 | 뜻 | 판정 |
|---|---|---|
| `기타`(`Category.OTHER`) | 사람이 "나열된 어디에도 안 맞는다"고 **확정**(폐기된 `비품`도 여기로) | 통과(과목 그래프 없으면 `NO_SCOPE_RULE_GRAPH`) |
