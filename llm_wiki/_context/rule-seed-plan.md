# Rule 시드 데이터 생성 계획 (RULE 명세서 → RuleGraph 시드) — 2026-07-30 스냅샷

> ⚠️ **전 구간 대체됨.** 이 문서는 "GLOBAL 게이트(R-002·R-003)만 구현, 엔진 부재"였던 2026-07-30 시점 계획이다.
> 이후 `seed_rules.py`가 GLOBAL + 접대·회식·출장 4개 계열 그래프 시드를 실제 DSL·결정론적 라우팅으로
> 구현했고, 판정 엔진도 `policies/orchestrator.py`로 구현됐다(둘 다 이 문서가 "미구현"이라 적은 것).
> **현재 상태는 `CLAUDE.md` §3.6(시드)·`_context/default-gate.md`·`_context/rule-engine.md`가 정본이다.**
> 아래 본문은 초기 설계 의도 기록 목적으로만 남긴다 — 특히 "회식은 독립 scope"라는 서술은
> 2026-08-14 카테고리 개편으로 무효가 됐다(회식은 이제 실제로 독립 scope다, 당시엔 아니었다).
