...
| `_context/agent-v1-upgrade-plan.md` | **Rule Agent·Risk Review Agent v1 고도화 — 결정 사항 요약(경량화판)** — 구현 근거·코드 위치·검증 결과는 전부 구현 기록 문서로 넘기고 "무엇을 결정했는지"만 표로 정리. §1 Rule Agent 6개 항목 결정 요약, §2 Risk Review Agent 미착수 항목, §3a 프론트 격리, **§4 테스트케이스 자동생성 — 구현+검증 완료**: 대화형 아닌 완제품 생성, 커버리지 기반+자체검증 루프 두 축, append·노드당 2건 확정 | §1·§4 전부 결정+구현 완료, §2(Risk Review)만 미착수(2026-08-18) |
...
| `_context/rule-agent-v1-ux-upgrade-plan.md` | **시뮬레이션/검증셋 UX·품질 고도화 — §9 1~7번 전 항목 구현+검증 완료** — 모델 티어링(gpt-5-mini/gpt-4o-mini, temperature 미지원 버그 발견수정)·최근내역 문구요약+reversal등급·권장처리 원인배지·empty state·**검증셋 자동생성 타임아웃 수정(narrate 내부호출 생략+타임아웃 개별조정)**·**검증셋버튼 완전제거(실행 버튼 하나로 통합, mock 모달 삭제)**·**최소5건 미달 원인 수정(도달불가/종단노드 후보생성 스킵) + 그래프81 실제 라우팅버그 발견수정**·**append→replace 전환 + 상한10건**·**§2-2 생성 사유 로그(재시도가 죽은코드였던 것 발견수정)**·**§4 대화형수정에 시뮬레이션결과 컨텍스트 주입**·**§6 보고서에 구체사례 인용** | §1~7 전항목 구현+검증 완료(2026-08-19), §8만 후순위 백로그 |
