...
## 0. 폴더 소유권 — `docs/` vs `_context/`

| 위치 | 소유 | 성격 | 쓰는 규칙 |
|---|---|---|---|
| `docs/` | **팀(사람)** | 팀이 관리하는 **기준문서**. 요구사항·기술·기획 등 합의된 스펙과 보고서 | 에이전트는 **읽기 기본**. 결정이 바뀌면 사람이 합의한 뒤 본문을 고치고 이 색인 행을 같이 갱신 |
| `_context/` | **AI(에이전트)** | 에이전트가 만들고 유지하는 **AI용 파생 컨텍스트**(캐논·설계·구현 추적) | 에이전트가 자유롭게 생성·갱신. 기준문서와 **중복 서술 금지**(파생/요약/구현 상세만) |
| `화면설계서/` · `figma_mockup/` | 팀 | 화면 산출물(.docx 압축해제 · 목업 SVG) | 프론트 구현 기준. 픽셀 매칭 불필요 |

> 두 층이 어긋나면 **`docs/`가 이긴다.** `_context/`가 앞서 나간 내용(구현이 먼저 된 결정)은 해당 문서 상단에 "기준문서 미반영" 표기를 남기고, 확정되면 `docs/`로 승격한다.

## 1. 권위 스펙 문서 (Source of Truth) — `docs/`

| 문서 | 버전 | 권위 범위 | 상태 | 최종 갱신 |
|---|---|---|---|---|
| `docs/요구사항_명세서.md` | Draft v0.5 | 기능/비기능 요구사항(FR-*), 상태머신, Open Issue | 최신 · 제출 2단계·팀 배치·분류 정합 갭 명시(FR-ST-05·Open #8~9) | 2026-07-31 |
| `docs/기술명세서.md` | Draft v0.2 | 아키텍처·데이터·API·FastMCP Tool·ML/RAG·룰 그래프 | 최신 · §3.3 제출플로우/계정과목 구현 정합 갭 명시 | 2026-07-31 |
| `docs/기획_확장안_v2.md` | Draft v0.2 | 제품 기획·3-Agent 플로우·객체 모델·라이프사이클 | 최신 · §1.5 1인 팀 경로·정합 갭 크로스레퍼런스 | 2026-07-31 |
| `화면설계서/` | Rev.1 v1.1 | 6개 화면(S-01~06)·역할·상태머신 화면매핑 (압축해제 .docx) | 프론트 구현 기준 | — |
| `docs/법인카드_사용규정_기반_RULE_명세서.md` | v1.4 | **참고용 예시** — 규정에서 도출 가능한 58 RULE을 사람이 손으로 뽑아 본 산출물. 필드정의·심각도·우선순위 | ⚠️ **권위 스펙 아님.** 제품 기본 제공은 **DEFAULT GATE 1개**뿐이고 세부 룰은 **고객 규정 문서 업로드 시 생성**된다. 필드 정의는 이상 집합이며 실제 스키마는 v3(46) | 2026-08-11 |

> 세 스펙 문서(요구사항·기술·기획)는 서로 **상충 없이** 유지한다. 상태머신·룰 도메인·Risk 2단계 등 핵심 결정은 `CLAUDE.md §2`에 요약.
> ⚠️ 스펙 문서 3종은 **2026-07-31 기준**이라, 그 이후의 큰 결정(정책 도메인 재구축·EvalContext v4·PDF 파싱/청킹/임베딩)은 아직 `_context/`에만 있다. 본문 인용 시 §3 표와 대조할 것.
...
> 에이전트가 생성·유지하는 AI용 컨텍스트. 팀 기준문서(`docs/`)와 충돌하면 기준문서가 우선이며, 여기서 먼저 확정된 결정은 승격 대상으로 표기한다.
...
| 문서 | 용도 | 상태 |
|---|---|---|
| `docs/비지도모델_비교실험_결과보고서.md` | 비지도 이상탐지 8종(IF·COPOD·ECOD·CBLOF·PCA·INNE·GMM·LODA) 동일 조건 비교 + 5-fold 재검증 → **IF 유지, COPOD는 설명가능성 대안** | 2026-08-03 · LODA 부호·쌍대 차이 재계산 확인 대기 |
| `../scrum/중간발표/중간발표_참고용_context.md` | 중간발표(2026-08-06) 슬라이드 작성용 원본 컨텍스트 — 문제정의·근거자료·해결방안·아키텍처·기대효과(기획/엔지니어링 2버전)·진행상황·자체평가 | 2026-08-03 · **llm_wiki 밖 `scrum/`으로 이동** |
| `../scrum/중간발표/` · `../scrum/daily_scrum/` | 중간발표 대본·최종 PDF, 주차별 스크럼(2~5주차) | 발표·진행보고 자료 |

## 5. 데이터/자산 위치 (문서 아님)

- `../tiger_inc/` — **RAG 소스 데이터**(사내 규정·조직). ⚠️ 직접 문서작업 외 열람 금지(`CLAUDE.md §5`).
- `figma_mockup/` — 화면 목업 SVG(참고용).
- `../docling_eval/` — RAG 파싱·청킹·임베딩 **평가 노트북과 산출물**(`output/`). §3의 파싱/청킹/임베딩 캐논의 실측 근거.
