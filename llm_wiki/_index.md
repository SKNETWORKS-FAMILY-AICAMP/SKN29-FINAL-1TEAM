...
| `_context/document-triage.md` | **문서 분류(triage) + 별표 승인 캐논** — 업로드된 규정 문서를 한 번 읽고 갈라주는 단계. 조항 성격(규정/안내/별표참조)과 룰 생성 우선순위(AUTO/1~3순위/제외)를 매기고, 별표를 `PolicyTable` 후보로 뽑아 **승인 화면**을 거쳐 임계값으로 승격한다. **분류는 제안이지 차단이 아니다**(SKIP 조항도 룰 생성 가능) · 후보는 `PolicyTable`과 별도 모델 · 승인이 강제하는 건 축뿐(값은 표 원문 대조) · AUTO가 없으면 자동 생성 안 함 | 구현 완료 (2026-08-22). 회귀 core 19 + ai 12. 실 문서 end-to-end는 도커·API 키 필요 |
