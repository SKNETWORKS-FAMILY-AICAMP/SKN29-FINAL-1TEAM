...
- **비용분류 어휘 = 정본 1곳 + 서버가 내려준다**: 정본 `settlements.Category` 7종(회식·회의·식대·출장·접대·비품·**기타**), 노출 창구는 `GET /api/meta/categories/` 하나이고 화면(`useCategories()`)·ai(`core_client.get_categories()`)가 **런타임에 받아 쓴다** — 상수 복사본 9곳을 걷어낸 이유는 "업무활성"→"회식" 리네임 때 손으로 따라가다 두 번 어긋난 이력이다. **저장 검증은 서버가 한다**(`choices=`는 DB 제약이 아니고 커스텀 create/update는 `full_clean()`을 안 부른다 → 목록 밖 값은 400). 룰 그래프 scope는 `GLOBAL ∪ Category`이며 규정 문서 표기(기업업무추진비 등)는 `policies/scope.normalize_scope`가 접는다. **`기타` ≠ 미기재** — `기타`는 "어디에도 안 맞는다"는 확정이고 `""`는 "아직 못 정했다"(기본 게이트 `CATEGORY_MISSING` → 검토). AI가 특정하지 못하면 실재 과목으로 밀지 않고 비워 둔다. — `_context/category-vocabulary.md` / 요구사항 §4.2 / 기술 §3.3
- **초안 Agent는 판정을 예측하지 않는다**: 「지금 제출하면 보완요청/반려될까」는 결정론적 엔진이 이미 답을 갖고 있다 (`orchestrator.judge(record=False)` — 이 용도로 만들어 두고 호출부가 0건이던 통로). 룰 그래프를 프롬프트에 주고 순회를 흉내내게 하면 틀리고, 틀려도 티가 안 나며, 사용자에겐 "AI가 통과라 했는데 반려됨"이 된다. **엔진이 결정하고 모델은 사람 말로 옮긴다**(`narrate.py`·MCP `run_rule_engine`과 같은 분업). 모델이 낼 수 없어야 하는 것(가맹점·금액·일시·업종)은 지시가 아니라 **출력 스키마에서 뺀다**. — `_context/draft-agent-v2.md` / 기술 §4.1
...
| `_context/draft-agent-v2.md` | Draft Agent v2 캐논 — 사실 주입(기본 내역·첨부 추출·EvalContext)과 **엔진 dry-run으로 얻는 판정 미리보기**, 룰 그래프를 LLM에 주고 예측시키지 않는 이유(D-1), 모델이 낼 수 없는 것은 스키마에서 뺀다(D-2), 영수증→거래 원장 반영 규칙(D-5), 저장 먼저 흐름(D-6), 제출 시 문체 다듬기와 기계 대조(D-8) | 구현 완료 (2026-08-22, 회귀 core 21 + ai 20) |
| `_context/category-vocabulary.md` | 비용분류 어휘 정본 캐논 — 상수 복사본 9곳을 서버 단일 창구(`/api/meta/categories/`)로 수렴시킨 배경과 결정 D-1~D-6, ai 구조화 출력 enum 런타임 재생성, `기타`와 미기재를 가르는 이유, 함께 고친 화면 결함 3건, 분류 추가 체크리스트 | 구현 완료 (`CategoryMetaView`, 마이그레이션 `policies/0019`·`settlements/0013`, 회귀 core 15 + ai 8) |
...
| `_context/policy-domain.md` | 규정 임계값(policy) 도메인 캐논 — 저장층(`policy_tables` 자유 JSON)/소비층(`ctx.policy.*` 고정 카탈로그)/해소 규약 2층 구조, 미해소 가드 | 구현 완료 (`policies/context_builder.py`·`tiger_tables.py`, EvalContext v2) | · **2026-08-22 §3.1~3.3 추가**: `ctx.policy.*`를 고정 8칸에서 **적재된 별표에서 파생**으로 전환(임계값은 열고 사실은 닫는다) + 축 정합 검사(`check_table_axes`) + 별표 적재 파이프라인을 다음 작업으로 명시
