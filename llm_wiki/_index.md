...
| `docs/qa/rule-engine-qa.md` | 2026-08-25 | **룰엔진 검증** — 도입 타임라인 7단계 × 검증셋 300건. 자동처리율·오탐율 실측, 최종 그래프 구조, 잡은 결함 6건 | 확정 |
...
| `rule-engine-semantics.md` | **판정 의미론(불변식)** — 룰엔진은 분류기다 · 검토는 실패가 아니다 · 승인대기는 적극적 결정(디폴트 PASS 금지) · 자동처리·오탐·미탐의 정의. §6에 이 원칙에 반한 결함 4건 |
...
| `rag-ingestion.md` | 규정 문서 적재 파이프라인 + 룰 트리거 · docling 모킹 스위치(§3b — 신규 문서 덤프 추가 포함) |
